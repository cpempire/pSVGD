...
for case in [0, 1, 2, 3]:
    ...
    for i in range(Ntrial):
        filename = "accuracy-" + str(d) + 'x' + str(d) + "/data/data_nSamples_" + str(N) + "_isProjection_" + str(True) + "_SVGD_" + str(i+1) + ".p"
        ...
        filename = "accuracy-" + str(d) + 'x' + str(d) + "/data/data_nSamples_" + str(N) + "_isProjection_" + str(False) + "_SVGD_" + str(i+1) + ".p"

        data_save = pickle.load(open(filename, 'rb'))
        meanErrorL2normFalse_i = data_save["meanErrorL2norm"]
        varianceErrorL2normFalse_i = data_save["varianceErrorL2norm"]
        # if len(meanErrorL2normFalse_i) == 100:
        #     help = np.array([meanErrorL2normFalse_i, meanErrorL2normFalse_i]).reshape(200,)
        #     meanErrorL2normFalse_i = help
        #     help = np.array([varianceErrorL2normFalse_i, varianceErrorL2normFalse_i]).reshape(200,)
        #     varianceErrorL2normFalse_i = help
...
for i in range(Ntrial):
    ...
    plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(meanErrorDimensionFalse[:, i]), makerFalse[case], alpha=0.2)
    plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(meanErrorDimensionTrue[:, i]), makerTrue[case], alpha=0.2)

# plt.plot(np.log2(dPara), np.log10(np.sqrt(np.mean(meanErrorDimensionSVGD ** 2, axis=1))), makerRMSEFalse[1], linewidth=2,
#          label='SVGD')
plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(np.sqrt(np.mean(meanErrorDimensionFalse ** 2, axis=1))), makerRMSEFalse[case], linewidth=2,
         label='SVGD')
plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(np.sqrt(np.mean(meanErrorDimensionTrue ** 2, axis=1))), makerRMSETrue[case], linewidth=2,
         label='pSVGD')
plt.figure(1)
plt.legend(fontsize=16)
plt.xlabel("$\log_2(\sqrt{d}-1)$", fontsize=16)
plt.ylabel("$\log_{10}$(RMSE of mean)", fontsize=16)

plt.tick_params(axis='both', which='major', labelsize=12)
plt.tick_params(axis='both', which='minor', labelsize=12)

filename = "figure/error_mean_dimension_nonlinear"+"SVGD.pdf"
fig1.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_mean_dimension_nonlinear"+"SVGD.eps"
fig1.savefig(filename, format='eps', bbox_inches='tight')
...
for i in range(Ntrial):
    ...
    plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(varianceErrorDimensionFalse[:, i]), makerFalse[case], alpha=0.2)
    plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(varianceErrorDimensionTrue[:, i]), makerTrue[case], alpha=0.2)
# plt.plot(np.log2(dPara), np.log10(np.sqrt(np.mean(varianceErrorDimensionSVGD ** 2, axis=1))), makerRMSEFalse[1], linewidth=2,
#          label='SVGD')
plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(np.sqrt(np.mean(varianceErrorDimensionFalse ** 2, axis=1))), makerRMSEFalse[case], linewidth=2,
         label='SVGD')
plt.plot(np.log2(np.sqrt(dPara)-1), np.log10(np.sqrt(np.mean(varianceErrorDimensionTrue ** 2, axis=1))), makerRMSETrue[case], linewidth=2,
         label='pSVGD')
plt.legend(fontsize=16)
plt.xlabel("$\log_2(\sqrt{d}-1)$", fontsize=16)
...
filename = "figure/error_variance_dimension_nonlinear"+"SVGD.pdf"
fig2.savefig(filename, format='pdf', bbox_inches='tight')
filename = "figure/error_variance_dimension_nonlinear"+"SVGD.eps"
fig2.savefig(filename, format='eps', bbox_inches='tight')
